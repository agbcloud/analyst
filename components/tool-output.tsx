...
interface ToolInvocationResult {
  toolName?: string;
  result?: {
    error?: { name?: string; traceback?: string };
    results?: Result[];
  };
}
...
export function ToolOutput({ result }: { result: ToolResult | undefined }) {
  ...
  const toolResult = (result as ToolInvocationResult[]).find((r) => r.toolName === "runCode")?.result;
        ...
        <div className="flex justify-end border rounded-lg overflow-hidden">
          <button
            className={`px-3 py-2 font-semibold text-sm ${
              viewMode === "static" ? "bg-primary/10 text-primary" : ""
            }`}
